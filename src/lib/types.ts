...
/**
 * 2D point in layout or canvas space.
 */
...
export const COLORS = {
  ...
  /**
   * Output Z terminal fill when HIGH / LOW.
   */
  outputOn: '#39ff14',
  outputOff: '#1a5f1a',
...
/**
 * Character range in the original expression string (for hover highlighting).
 */
...
/**
 * Supported gate symbols in layout and canvas drawing.
 */
export type GateType = 'NOT' | 'AND' | 'OR' | 'XOR';

/**
 * One placed gate: position, pin geometry, and the sub-expression it implements.
 */
...
/**
 * Vertical input rail for one variable.
 */
...
/**
 * One straight wire segment (horizontal or vertical) with optional rail jumpers.
 */
...
/**
 * Logical wire from a source (rail, gate, or output) to a destination pin.
 */
...
/**
 * Point where a variable leaves its vertical rail onto a horizontal wire.
 */
...
/**
 * Screen position of an HTML input switch overlay (below the rail label).
 */
...
/**
 * Input variable label above a rail.
 */
...
/**
 * Complete circuit graph for drawing and simulation.
 */
